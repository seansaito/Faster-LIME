...
class Adversarial_Lime_Model(Adversarial_Model):
    ...
    def train_ctgan(self, X, y, ctgan_sampler, feature_names, perturbation_multiplier=30,
        ...
        if estimator is not None:
            ...
            self.perturbation_identifier = RandomForestClassifier(n_estimators=rf_estimators).fit(
                xtrain, ytrain)
    ...
    def train(self, X, y, feature_names, perturbation_multiplier=30, categorical_features=[],
              rf_estimators=100, estimator=None):
...
class Adversarial_Kernel_SHAP_Model(Adversarial_Model):
    ...
    def train(self, X, y, feature_names, background_distribution=None, perturbation_multiplier=10,
              n_samples=2e4, rf_estimators=100, n_kmeans=10, estimator=None):
        ...
        for _ in range(int(n_samples)):
            ...
            for _ in range(X.shape[1]):
                ...
                point[j] = deepcopy(
                    background_distribution[np.random.choice(background_distribution.shape[0]), j])
        ...
        xtrain, xtest, ytrain, ytest = train_test_split(all_instances_x, all_instances_y,
                                                        test_size=0.2)
